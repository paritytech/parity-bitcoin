...
impl ChainClient {
			...
			while !thread_stop.load(Ordering::SeqCst) {
				match thread_queue.verify(&*thread_verifier) {
					TaskResult::Ok => {
						...
						trace!(target: "verificatino_queue", "Kicked insert thread...");
					},
					TaskResult::Wait => {
						trace!(target: "verification_queue", "Waiting for verify...");
						...
						*kick = false;
						...
						trace!(target: "verification_queue", "Done waiting for verify...");
					},
				}
			}
			...
			while !thread_stop.load(Ordering::SeqCst) {
				match thread_queue.fetch(&*thread_store) {
					TaskResult::Ok => {
						...
						trace!(target: "verification_queue", "Kicked verification thread...");
					},
					TaskResult::Wait => {
						trace!(target: "verification_queue", "Waiting for fetch...");
						...
						*kick = false;
						...
						trace!(target: "verification_queue", "Done waiting for fetch...");
					},
				}
			}
	...
	fn insert_thread(&self) -> JoinHandle<()> {
		...
		thread::Builder::new().name("Insert thread".to_owned()).spawn(move || {
			while !thread_stop.load(Ordering::SeqCst) {
				trace!(target: "verification_queue", "Inserting block...");
				match thread_queue.insert_verified(&*thread_store) {
					...
					TaskResult::Wait => {
						...
						*kick = false;
						...
						trace!(target: "verification_queue", "Done waiting for insert...");
					}
				}
			}
...
impl Drop for ChainClient {
	fn drop(&mut self) {
		...
		trace!(target: "verification_queue", "Finalized fetch threads");
